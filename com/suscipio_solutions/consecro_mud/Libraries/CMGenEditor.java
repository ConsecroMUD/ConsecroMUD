...
import com.suscipio_solutions.consecro_mud.core.CMSecurity.SecGroup;
...
public class CMGenEditor extends StdLibrary implements GenericEditor
{
	...
	protected void genReadable1(MOB mob, Item E, int showNumber, int showFlag)
	{
		...
		||(E instanceof Recipe)
		||(E instanceof com.suscipio_solutions.consecro_mud.Items.interfaces.RoomMap))
		...
		throws IOException
	{
		 ...
		 ||(E instanceof DoorKey))
		{
			...
			while((mob.session()!=null)&&(!mob.session().isStopped())&&(!ok))
			{
				...
				if(E instanceof com.suscipio_solutions.consecro_mud.Items.interfaces.RoomMap)
				{
		...
		throws IOException
	{
		try
		{
			...
			if(E instanceof com.suscipio_solutions.consecro_mud.Items.interfaces.RoomMap)
				modifyGenMap(mob,(com.suscipio_solutions.consecro_mud.Items.interfaces.RoomMap)E);
			else
			...
			if(itemstr.length()>0)
			{
				...
				else
				{
					...
					else
					{
						...
						if((item!=null)&&((!(item instanceof ImmortalOnly))||(CMSecurity.isASysOp(mob))))
						{
		...
		while((mob.session()!=null)&&(!mob.session().isStopped())&&(behave.length()>0))
		{
			...
			if(behave.length()>0)
			{
				...
				else
				{
					...
					{
						chosenOne=CMClass.getAbility(behave);
						if((chosenOne!=null)&&((chosenOne.classificationCode()&Ability.ALL_DOMAINS)==Ability.DOMAIN_IMMORTAL)&&(!CMSecurity.isASysOp(mob)))
							chosenOne=null;
						if(chosenOne!=null)
						{
	...
	protected void modifyGenMap(MOB mob, com.suscipio_solutions.consecro_mud.Items.interfaces.RoomMap me)
		throws IOException
	{
		if(mob.isMonster())
			return;
