...
public class CMFilter extends StdLibrary implements TelnetFilter
{
	@Override public String ID(){return "CMFilter";}
	...
	public String simpleOutFilter(String msg)
	{
		...
		if(CMSecurity.isDebugging(CMSecurity.DbgFlag.OUTPUT))
			Log.debugOut("CMFilter","OUTPUT: ?: "+buf.toString());
		return buf.toString();
	}
	...
	public String colorOnlyFilter(String msg, Session S)
	{
		...
		if(CMSecurity.isDebugging(CMSecurity.DbgFlag.OUTPUT))
			Log.debugOut("CMFilter","OUTPUT: "+(((S!=null)&&(S.mob()!=null))?S.mob().Name():"")+": "+buf.toString());
		return buf.toString();
	}
								...
								boolean wrapOnly)
	{
		...
		while(buf.length()>loop)
		{
			if(loop==lastLoop)
			{
				...
				{
					Log.debugOut("CMFilter","LOOP: "+loop+"/"+wrap+"/!"+(buf.charAt(loop)=='\033')+"!/"+lastSpace+"/"+firstAlpha+"/"+amperStop+"/"+doSagain+"/"+firstSdone+"/"+buf.length()+"/"+loopDebugCtr);
					Log.debugOut("CMFilter",buf.toString());
					break;
				}
			}
			else
