...
import com.suscipio_solutions.consecro_mud.core.MiniJSON;
...
public class CMJournals extends StdLibrary implements JournalsLibrary
{
	...
	{
		HashSet<String> H = (HashSet<String>)Resources.getResource("IMMORTAL_ONLY_JOURNALS");
		if(H == null)
		{
			Item I=null;
			...
			{
				I=e.nextElement();
				if((I instanceof ImmortalOnly)
				&&(!I.isGeneric()))
					H.add(I.Name().toUpperCase().trim());
			}
			Resources.submitResource("IMMORTAL_ONLY_JOURNALS", H);
		}
		return H;
	}
	...
	{
		if(getImmortalJournalNames().contains(journal.toUpperCase().trim()))
			return true;
		return false;
	}
