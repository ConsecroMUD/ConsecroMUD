...
import com.suscipio_solutions.consecro_mud.Areas.interfaces.AutoGenArea;
...
import com.suscipio_solutions.consecro_mud.core.collections.STreeSet;
...
import com.suscipio_solutions.consecro_mud.core.interfaces.Economics;
...
public class CMMaker extends StdLibrary implements GenericBuilder
{
	@Override public String ID(){return "CMMaker";}
	...
	public String getPropertiesStr(Environmental E, boolean fromTop)
	{
		if(E==null)
		{
			Log.errOut("CMMaker","getPropertiesStr: null 'E'");
			return "";
		}
	...
	{
		Log.errOut("CMMaker","unpack"+where+"FromXML: "+msg);
		return msg;
	}
		...
		if(andContent)
		{
			...
			if(cV.size()>0)
			{
				...
				for(int i=0;i<iV.size();i++)
				{
					...
					if(newItem instanceof ImmortalOnly) continue;
		...
		if((newArea!=null)&&(!newArea.getParents().hasMoreElements()))
		{
			...
			if((defaultParentAreaName!=null)&&(defaultParentAreaName.trim().length()>0))
			{
				...
				if(defaultParentArea==null)
					Log.errOut("RoomLoader","Default parent area from consecromud.ini '"+defaultParentAreaName.trim()+"' is unknown.");
			}
			if(defaultParentArea!=null)
			{
	...
	public void logDiff(Environmental E1, Environmental E2)
	{
		...
		if(Log.debugChannelOn())
			Log.debugOut("CMMaker",str.toString());
	}
	...
	public String addItemsFromXML(List<XMLpiece> xml, List<Item> addHere, Session S)
	{
		...
		for(int i=0;i<iV.size();i++)
		{
			...
			final String itemClass=CMLib.xml().getValFromPieces(iblk.contents,"ICLAS");
			final Item newItem=CMClass.getItem(itemClass);
			if((newItem instanceof ImmortalOnly)
	...
	public void setPropertiesStr(Environmental E, String buf, boolean fromTop)
	{
		...
		if(V==null)
			Log.errOut("CMMaker","setPropertiesStr: null 'V': "+((E==null)?"":E.Name()));
		else
	...
	public void setPropertiesStr(Environmental E, List<XMLpiece> V, boolean fromTop)
	{
		if(E==null)
		{
			Log.errOut("CMMaker","setPropertiesStr2: null 'E'");
			return;
		}
	...
	protected void setOrdPropertiesStr(Environmental E, List<XMLpiece> V)
	{
		if(V==null)
		{
			Log.errOut("CMMaker","null XML returned on "+identifier(E,null)+" parse. Load aborted.");
			return;
		}
		...
		if(E instanceof Area)
		{
			...
			if(VP!=null)
			{
				for(int i=0;i<VP.size();i++)
				{
					...
					{
						Log.errOut("CMMaker","Error parsing 'PARENT' of "+identifier(E,null)+".  Load aborted");
						return;
					}
					final String aName=CMLib.xml().getValFromPieces(ablk.contents,"PARENTNAMED");
					final Area A=CMLib.map().getArea(aName);
					if(A==null)
						Log.errOut("CMMaker","Unknown 'PARENT' area "+identifier(E,null)+": "+aName);
					else
					{
						((Area)E).addParent(A);
						A.addChild((Area)E);
			...
			if(VC!=null)
			{
				for(int i=0;i<VC.size();i++)
				{
					...
					{
						Log.errOut("CMMaker","Error parsing 'CHILD' of "+identifier(E,null)+".  Load aborted");
						return;
					}
					final String aName=CMLib.xml().getValFromPieces(ablk.contents,"CHILDNAMED");
					final Area A=CMLib.map().getArea(aName);
					if(A==null)
						Log.errOut("CMMaker","Unknown 'CHILD' area "+identifier(E,null)+": "+aName);
					else
					{
						((Area)E).addChild(A);
						A.addParent((Area)E);
	...
	protected void setGenMobAbilities(MOB M, List<XMLLibrary.XMLpiece> buf)
	{
		...
		if(V==null)
		{
			Log.errOut("CMMaker","Error parsing 'ABLTYS' of "+identifier(M,null)+".  Load aborted");
			return;
		}
		for(int i=0;i<V.size();i++)
		{
			...
			{
				Log.errOut("CMMaker","Error parsing 'ABLTY' of "+identifier(M,null)+".  Load aborted");
				return;
			}
			final Ability newOne=CMClass.getAbility(CMLib.xml().getValFromPieces(ablk.contents,"ACLASS"));
			if(newOne==null)
			{
				Log.errOut("CMMaker","Unknown ability "+CMLib.xml().getValFromPieces(ablk.contents,"ACLASS")+" on "+identifier(M,null)+", skipping.");
				continue;
			}
			final List<XMLLibrary.XMLpiece> adat=CMLib.xml().getContentsFromPieces(ablk.contents,"ADATA");
			if(adat==null)
			{
				Log.errOut("CMMaker","Error parsing 'ABLTY DATA' of "+identifier(M,null)+".  Load aborted");
				return;
			}
	...
	public void setGenScripts(PhysicalAgent E, List<XMLpiece> buf, boolean restoreVars)
	{
		...
		for(int i=0;i<V.size();i++)
		{
			...
			{
				Log.errOut("CMMaker","Error parsing 'SCRPT' of "+identifier(E,null)+".  Load aborted");
				return;
			}
			...
			if(script==null)
			{
				Log.errOut("CMMaker","Error parsing 'SCRIPT' of "+identifier(E,null)+".  Load aborted");
				continue;
			}
	...
	protected void setGenMobInventory(MOB M, List<XMLpiece> buf)
	{
		...
		if(V==null)
		{
			Log.errOut("CMMaker","Error parsing 'INVEN' of "+identifier(M,null)+".  Load aborted");
			return;
		}
		...
		for(int i=0;i<V.size();i++)
		{
			...
			if((!iblk.tag.equalsIgnoreCase("ITEM"))||(iblk.contents==null))
			{
				Log.errOut("CMMaker","Error parsing 'ITEM' of "+identifier(M,null)+".  Load aborted");
				return;
			}
			final Item newOne=CMClass.getItem(CMLib.xml().getValFromPieces(iblk.contents,"ICLASS"));
			if(newOne instanceof ImmortalOnly) continue;
			if(newOne==null)
			{
				Log.errOut("CMMaker","Unknown item "+CMLib.xml().getValFromPieces(iblk.contents,"ICLASS")+" on "+identifier(M,null)+", skipping.");
				continue;
			}
			final List<XMLLibrary.XMLpiece> idat=CMLib.xml().getContentsFromPieces(iblk.contents,"IDATA");
			if(idat==null)
			{
				Log.errOut("CMMaker","Error parsing 'ITEM DATA' of "+identifier(M,null)+".  Load aborted");
				return;
			}
	...
	public void populateShops(Environmental E, List<XMLpiece> buf)
	{
		...
		if(V==null)
		{
			Log.errOut("CMMaker","Error parsing 'STORE' of "+identifier(E,null)+".  Load aborted");
			return;
		}
		final Hashtable<String,Container> IIDmap=new Hashtable<String,Container>();
		final Hashtable<Item,String> LOCmap=new Hashtable<Item,String>();
		for(int i=0;i<V.size();i++)
		{
			...
			{
				Log.errOut("CMMaker","Error parsing 'SHITEM' of "+identifier(E,null)+".  Load aborted");
				continue;
			}
			...
			if(newOne==null)
			{
				Log.errOut("CMMaker","Unknown item "+itemi+" on "+identifier(E,null)+", skipping.");
				continue;
			}
			if(idat==null)
			{
				Log.errOut("CMMaker","Error parsing 'SHOP DATA' of "+identifier(E,null)+".  Load aborted");
				continue;
			}
		...
		&&(P.isGeneric()))
		{
			...
			if(cataP!=null)
			{
				if(CMath.bset(cataP.basePhyStats().disposition(),PhyStats.IS_CATALOGED))
					Log.errOut("CMMaker","Error with catalog object "+P.Name()+".");
				else
				if(cataP!=P)
				{
					if(fromTop)
	...
	protected void setGenPropertiesStr(Environmental E, List<XMLpiece> buf)
	{
		...
		{
			Log.errOut("CMMaker","MOB "+identifier(E,null)+" has malformed XML. Load aborted.");
			return;
		}
			...
			try
			{
				...
				if(mobName.length()>0)
				{
					...
					if((dblk!=null)&&(dblk.size()>0))
					{
						...
						if(newOne==null)
							Log.errOut("CMMaker","Unknown tool "+itemi+" of "+identifier(E,null)+".  Skipping.");
						else
						{
							setPropertiesStr(newOne,idat,true);
		...
		if(E instanceof MOB)
		{
			...
			if(E instanceof Deity)
			{
				...
				if(V==null)
				{
					Log.errOut("CMMaker","Error parsing 'BLESSINGS' of "+identifier(E,null)+".  Load aborted");
					return;
				}
				for(int i=0;i<V.size();i++)
				{
					...
					{
						Log.errOut("CMMaker","Error parsing 'BLESS' of "+identifier(E,null)+".  Load aborted");
						return;
					}
					final Ability newOne=CMClass.getAbility(CMLib.xml().getValFromPieces(ablk.contents,"BLCLASS"));
					if(newOne==null)
					{
						Log.errOut("CMMaker","Unknown bless "+CMLib.xml().getValFromPieces(ablk.contents,"BLCLASS")+" on "+identifier(E,null)+", skipping.");
						continue;
					}
					...
					if(adat==null)
					{
						Log.errOut("CMMaker","Error parsing 'BLESS DATA' of "+identifier(E,null)+".  Load aborted");
						return;
					}
					setPropertiesStr(newOne,adat,true);
				...
				if(V!=null)
				{
					for(int i=0;i<V.size();i++)
					{
						...
						{
							Log.errOut("CMMaker","Error parsing 'CURSE' of "+identifier(E,null)+".  Load aborted");
							return;
						}
						final Ability newOne=CMClass.getAbility(CMLib.xml().getValFromPieces(ablk.contents,"CUCLASS"));
						if(newOne==null)
						{
							Log.errOut("CMMaker","Unknown curse "+CMLib.xml().getValFromPieces(ablk.contents,"CUCLASS")+" on "+identifier(E,null)+", skipping.");
							continue;
						}
						...
						if(adat==null)
						{
							Log.errOut("CMMaker","Error parsing 'CURSE DATA' of "+identifier(E,null)+".  Load aborted");
							return;
						}
						setPropertiesStr(newOne,adat,true);
				...
				if(V!=null)
				{
					for(int i=0;i<V.size();i++)
					{
						...
						{
							Log.errOut("CMMaker","Error parsing 'POWER' of "+identifier(E,null)+".  Load aborted");
							return;
						}
						final Ability newOne=CMClass.getAbility(CMLib.xml().getValFromPieces(ablk.contents,"POCLASS"));
						if(newOne==null)
						{
							Log.errOut("CMMaker","Unknown power "+CMLib.xml().getValFromPieces(ablk.contents,"POCLASS")+" on "+identifier(E,null)+", skipping.");
							continue;
						}
						final List<XMLLibrary.XMLpiece> adat=CMLib.xml().getContentsFromPieces(ablk.contents,"PODATA");
						if(adat==null)
						{
							Log.errOut("CMMaker","Error parsing 'POWER DATA' of "+identifier(E,null)+".  Load aborted");
							return;
						}
						setPropertiesStr(newOne,adat,true);
	...
	public void setExtraEnvProperties(Environmental E, List<XMLpiece> buf)
	{
		...
		if(E instanceof Economics)
		{
			...
			if(iV!=null)
			{
				...
				for(int i=0;i<iV.size();i++)
				{
					...
					{
						Log.errOut("CMMaker","Error parsing 'IPRICS' of "+identifier(E,null)+".  Load aborted");
						continue;
					}
		...
		if(E instanceof PhysicalAgent)
		{
			...
			if(V==null)
			{
				Log.errOut("CMMaker","Error parsing 'BEHAVES' of "+identifier(E,null)+".  Load aborted");
				return;
			}
			for(int i=0;i<V.size();i++)
			{
				...
				{
					Log.errOut("CMMaker","Error parsing 'BHAVE' of "+identifier(E,null)+".  Load aborted");
					return;
				}
				...
				if(newOne==null)
				{
					Log.errOut("CMMaker","Unknown behavior "+CMLib.xml().getValFromPieces(ablk.contents,"BCLASS")+" on "+identifier(E,null)+", skipping.");
					continue;
				}
		...
		if(E instanceof Physical)
		{
			...
			if(V==null)
			{
				Log.errOut("CMMaker","Error parsing 'AFFECS' of "+identifier(E,null)+".  Load aborted");
				return;
			}
			for(int i=0;i<V.size();i++)
			{
				...
				{
					Log.errOut("CMMaker","Error parsing 'AFF' of "+identifier(E,null)+".  Load aborted");
					return;
				}
				final Ability newOne=CMClass.getAbility(CMLib.xml().getValFromPieces(ablk.contents,"ACLASS"));
				...
				if(newOne==null)
				{
					Log.errOut("CMMaker","Unknown affect "+CMLib.xml().getValFromPieces(ablk.contents,"ACLASS")+" on "+identifier(E,null)+", skipping.");
					continue;
				}
