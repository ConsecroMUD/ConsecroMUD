						 ...
						 String varifyable)
	{
		...
		while((t>=0)&&(t<varifyable.length()-1))
		{
			...
			case 'a':
				...
				//case 'a':
			case 'A':
				// unnecessary, since, in consecromud, this is part of the name
				break;
		...
		for(int si=1;si<script.size();si++)
		{
			...
			case 65: // MPQSET
			{
				...
				if(var.equalsIgnoreCase("STATISTICS")&&(val.equalsIgnoreCase("ACCEPTED")))
					CMLib.coffeeTables().bump(Q,CMTableRow.STAT_QUESTACCEPTED);
				else
				if(var.equalsIgnoreCase("STATISTICS")&&(val.equalsIgnoreCase("SUCCESS")||val.equalsIgnoreCase("WON")))
					CMLib.coffeeTables().bump(Q,CMTableRow.STAT_QUESTSUCCESS);
				else
				if(var.equalsIgnoreCase("STATISTICS")&&(val.equalsIgnoreCase("FAILED")))
					CMLib.coffeeTables().bump(Q,CMTableRow.STAT_QUESTFAILED);
				else
				{
					...
					if(lastKnownLocation!=null)
					{
						...
						for(int i=0;i<Is.size();i++)
						{
							if(Is.elementAt(i) instanceof Item)
							{
								...
								if((m!=null)&&(!(m instanceof ImmortalOnly)))
								{
									m=(Item)m.copyOf();
									m.recoverPhyStats();
				...
				if(lastKnownLocation!=null)
				{
					...
					for(int i=0;i<Is.size();i++)
					{
						if(Is.elementAt(i) instanceof Item)
						{
							...
							if((I!=null)&&(!(I instanceof ImmortalOnly)))
							{
								I=(Item)I.copyOf();
								I.recoverPhyStats();
