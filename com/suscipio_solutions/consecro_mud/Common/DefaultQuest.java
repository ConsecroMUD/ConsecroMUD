...
import com.suscipio_solutions.consecro_mud.core.collections.ReverseEnumeration;
...
public class DefaultQuest implements Quest, Tickable, CMObject
{
	...
	{
		if((!running())&&(!isCopy()))
			CMLib.coffeeTables().bump(this,CMTableRow.STAT_QUESTSTARTATTEMPT);
	...
	public boolean startQuestInternal()
	{
		...
		if(questState.error)
		{
			...
			CMLib.coffeeTables().bump(this,CMTableRow.STAT_QUESTFAILEDSTART);
		}
		else
		if(!questState.done)
	...
	public void stopQuest()
	{
		if((!stoppingQuest)&&(running()))
			CMLib.coffeeTables().bump(this,CMTableRow.STAT_QUESTSUCCESS);
		stopQuestInternal();
	}
	...
	public boolean tick(Tickable ticking, int tickID)
	{
		...
		if(running())
		{
			...
			if(ticksRemaining<0)
			{
				...
				{
					CMLib.coffeeTables().bump(this,CMTableRow.STAT_QUESTTIMESTOP);
					stopQuest();
				}
			}
		...
		else
		{
			if(startQuestOnTime())
			{
				CMLib.coffeeTables().bump(this,CMTableRow.STAT_QUESTTIMESTART);
			}
		}
