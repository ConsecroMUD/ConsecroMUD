...
public class Stat  extends Skills
{
	...
	public boolean showTableStats(MOB mob, int days, int scale, String rest)
	{
		...
		if(skillUse)
		{
			...
			for(int x=0;x<allSkills.size();x++)
			{
				...
				if((CharC==null)||(CMLib.ableMapper().getQualifyingLevel(CharC.ID(),true,A.ID())<0))
					continue;
				if(totals[x][CMTableRow.STAT_SKILLUSE]>0)
				{
					table.append(CMStrings.padRight(""+A.ID(),25)
							+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_SKILLUSE],10));
					if(cr) table.append("\n\r");
					cr=!cr;
				}
				x++;
		...
		if(questStats)
		{
			...
			for(int x=0;x<CMLib.quests().numQuests();x++)
			{
				...
				table.append(
						...
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTSTARTATTEMPT],5)
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTTIMESTART],5)
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTFAILEDSTART],5)
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTACCEPTED],5)
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTSUCCESS],5)
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTFAILED],5)
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTDROPPED],5)
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTTIMESTOP],5)
						+CMStrings.centerPreserve(""+totals[x][CMTableRow.STAT_QUESTSTOP],5));
				table.append("\n\r");
			}
			table.append("\n\r");
		}
		...
		while((V.size()>0)&&(curTime>(ENDQ.getTimeInMillis())))
		{
			...
			totals[CMTableRow.STAT_TICKSONLINE]=(totals[CMTableRow.STAT_TICKSONLINE]*CMProps.getTickMillis())/scale/(1000*60);
			...
			table.append(CMStrings.padRight(CMLib.time().date2DateString(curTime+1)+" - "+CMLib.time().date2DateString(lastCur-1),25)
						 +CMStrings.centerPreserve(""+totals[CMTableRow.STAT_LOGINS],5)
						 +CMStrings.centerPreserve(""+highestOnline,5)
						 +CMStrings.centerPreserve(""+avgOnline,5)
						 +CMStrings.centerPreserve(""+totals[CMTableRow.STAT_TICKSONLINE],5)
						 +CMStrings.centerPreserve(""+totals[CMTableRow.STAT_NEWPLAYERS],5)
						 +CMStrings.centerPreserve(""+totals[CMTableRow.STAT_DEATHS],5)
						 +CMStrings.centerPreserve(""+totals[CMTableRow.STAT_PKDEATHS],5)
						 +CMStrings.centerPreserve(""+totals[CMTableRow.STAT_CLASSCHANGE],5)
						 +CMStrings.centerPreserve(""+totals[CMTableRow.STAT_PURGES],5)
						 +CMStrings.centerPreserve(""+totals[CMTableRow.STAT_MARRIAGES],5)+"\n\r");
			if(scale==0) break;
		}
