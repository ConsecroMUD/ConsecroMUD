...
public class Import extends StdCommand
{
								...
								Vector commands)
	{
		...
		for(int m=0;m<mobData.size();m++)
		{
			...
			if(circleFormat)
			{
				...
				// ignore the above, consecromud does it better!
				final int numAbiles=M.numAbilities();
				M.baseCharStats().getMyRace().startRacing(M,false);
				//if(CMath.isSet(off,0)) // no area killers in consecromud
				//if(CMath.isSet(off,1)) // no circling in consecromud
	...
	private static void applyItemApplyCode(String codesLine, Item I, Environmental adjuster, Environmental caster, Environmental resister)
	{
		...
		case 5:
			...
			break;
		case 6:
			// consecromud doesn't play with sex
			break;
		case 7:
								...
								Vector commands)
	{
		...
		for(int o=0;o<objectData.size();o++)
		{
			...
			//if(extraFlag&4096)==4096) consecromud doesn't support rotting cargo

			if(CMath.isSet(extraFlag,14))
				CMLib.flags().setGettable(I,false);

			//if(extraFlag&16384)==16384) consecromud doesn't support rotting cargo
					...
					else
					{
						...
						else
						{
							...
							if(M instanceof ShopKeeper)
							{
								...
								if(((I.ID().equals("GenReadable"))
								||(I instanceof com.suscipio_solutions.consecro_mud.Items.interfaces.RoomMap))
