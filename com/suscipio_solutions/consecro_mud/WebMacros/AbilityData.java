			...
			if(A!=null)
			{
				...
				if(parms.containsKey("POSTCASTAFFECT"))
				{
					...
					for(final Enumeration<Ability> e=CMClass.abilities();e.hasMoreElements();)
					{
						...
						if(((A2.classificationCode()&Ability.ALL_DOMAINS)==Ability.DOMAIN_IMMORTAL)&&(!CMSecurity.isASysOp(mob)))
							continue;
				...
				if(parms.containsKey("POSTCASTABILITY"))
				{
					...
					{
						final Ability A2=e.nextElement();
						if(((A.classificationCode()&Ability.ALL_DOMAINS)==Ability.DOMAIN_IMMORTAL)&&(!CMSecurity.isASysOp(mob)))
							continue;
						final String AID=A2.ID();
						final String ANAME=A2.name();
						str.append("<OPTION VALUE=\""+AID+"\""+(list.contains(AID.toUpperCase())?" SELECTED":"")+">"+ANAME);
				...
				if(parms.containsKey("QLEVEL"))
				{
					...
					if((className!=null)&&(className.length()>0))
						...
						level=CMLib.ableMapper().getQualifyingLevel("Immortal",true,A.ID());
					str.append(level+", ");
				}
