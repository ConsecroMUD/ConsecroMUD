...
import com.suscipio_solutions.consecro_mud.core.collections.SPairList;
		...
		public CODES()
		{
			...
			synchronized(this)
			{
				...
				for(int i=0;i<addExtra.length + repExtra.length;i++)
				{
					...
					if(V.size()!=8)
					{
						Log.errOut("RawMaterial","Bad consecromud.ini material row (requires 8 elements, separated by ,): "+p);
						continue;
					}
					String type="ADD";
					...
					if(replace)
					{
						...
						{
							Log.errOut("RawMaterial","Unknown replaceable resource in consecromud.ini: "+stat);
							continue;
						}
					}
					...
					if(material==null)
					{
						Log.errOut("RawMaterial","Unknown material code in consecromud.ini: "+matStr);
						continue;
					}
