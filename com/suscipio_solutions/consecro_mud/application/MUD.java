...
import com.suscipio_solutions.consecro_mud.core.database.DBConnection;
import com.suscipio_solutions.consecro_mud.core.database.DBConnector;
import com.suscipio_solutions.consecro_mud.core.database.DBInterface;
...
import com.suscipio_solutions.consecro_mud.core.intermud.IMudClient;
import com.suscipio_solutions.consecro_mud.core.intermud.cm1.CM1Server;
import com.suscipio_solutions.consecro_mud.core.intermud.i3.IMudInterface;
...
import com.suscipio_solutions.consecro_mud.core.smtp.SMTPserver;
import com.suscipio_solutions.consecro_mud.core.threads.CMRunnable;
import com.suscipio_solutions.consecro_mud.core.threads.ServiceEngine;
...
{
	private static final float	  HOST_VERSION_MAJOR	= (float)0;
	...
	public void shutdown(Session S, boolean keepItDown, String externalCommand)
	{
		...
		interrupt(); // kill the damn immortal thread.
	}
	...
	private static class HostGroup extends Thread
	{
		...
		public void fatalStartupError(Thread t, int type)
		{
			...
			case 5:
				...
				break;
			default:
				errorInternal="Fatal error loading classes.  Make sure you start up consecromud from the directory containing the class files.";
				break;
			}
		...
		protected boolean initHost()
		{
			...
			if(!CMClass.loadAllConsecroMudClasses(page))
			{
			...
			if((tCode==MAIN_HOST)||(checkPrivate&&CMProps.isPrivateToMe("MAP")))
			{
				...
				if(!CMLib.map().roomIDs().hasMoreElements())
				{
					...
					room.setDescription(CMLib.lang().L("Brand new database room! You need to change this text with the MODIFY ROOM command.  If your character is not an Immortal, pick up the book you see here and read it immediately!"));
					CMLib.database().DBCreateRoom(room);
					final Item I=CMClass.getMiscMagic("ManualImmortal");
					room.addItem(I);
	...
	public static void main(String a[])
	{
		...
		if(iniFiles.size()==0) iniFiles.addElement("consecromud.ini");
		...
		while(!bringDown)
		{
			System.out.println();
			grpid=0;
			Log.sysOut(Thread.currentThread().getName(),"ConsecroMud v"+HOST_VERSION_MAJOR + "." + HOST_VERSION_MINOR);
			Log.sysOut(Thread.currentThread().getName(),"(C) 2009-2014 Suscipio Solutions");
			Log.sysOut(Thread.currentThread().getName(),"http://www.consecromud.org");
			CMLib.hosts().clear();
