...
public class InstrumentMaking extends CraftingSkill implements ItemCraftor
{
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget, boolean auto, int asLevel)
	{
		...
		int duration=4;
		final boolean immortal=CMSecurity.isASysOp(mob)||CMSecurity.isAllowed(mob,mob.location(),CMSecurity.SecFlag.ALLSKILLS);
		if(str.equalsIgnoreCase("list"))
		{
			...
			for(int r=0;r<recipes.size();r++)
			{
				...
				if(V.size()>0)
				{
					...
					&&((race.length()==0)||immortal||((" "+race+" ").toUpperCase().indexOf(" "+mob.charStats().getMyRace().ID().toUpperCase()+" ")>=0))
		...
		for(int r=0;r<matches.size();r++)
		{
			...
			if(V.size()>0)
			{
				...
				&&((parsedVars.autoGenerate>0)||(race.length()==0)||immortal||((" "+race+" ").toUpperCase().indexOf(" "+mob.charStats().getMyRace().ID().toUpperCase()+" ")>=0)))
				{
					foundRecipe=V;
					break;
				}
